...
{
    [Kingdee.BOS.Util.HotUpdate, Description("推送费用项目至OA")]
    public class ExpensePush : AbstractOperationServicePlugIn
    {
        ...
        public override void BeginOperationTransaction(BeginOperationTransactionArgs e)
        {
            foreach (DynamicObject o in e.DataEntitys)
            {
                ...
                if (opName.Equals("PushOA"))
                {
                    ...
                    DynamicObject rateId = o["RATE"] as DynamicObject;
                    string rateNumber = Convert.ToString(rateId["Number"]);
                    string allowereim = Convert.ToString(o["ALLOWREIM"]);
                    string description = Convert.ToString(o["Description"]);

                    mainTable.Add("sl", rateNumber);
                    mainTable.Add("sfkyybx", allowereim.Equals("True")?"1":"0");
                ...
                string results = Utils.PostUrl(Utils.pushFYXMurl, "datajson=" + dataJson.ToString());
