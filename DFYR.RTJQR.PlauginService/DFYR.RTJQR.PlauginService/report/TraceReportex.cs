...
{
    [Kingdee.BOS.Util.HotUpdate, Description("应付单跟踪表执行扩展")]
    public class TraceReportex : Kingdee.K3.FIN.AP.App.Report.TraceService
    {
        /// <summary>
        /// 主要取数逻辑方法
        /// </summary>
        ...
        private void setTmpData(string tableName, string tempTableName, IRptParams filter)
        {
            ...
            string ht = Convert.ToString(customFilter["F_PYEO_HTH_Id"]);
            ...
            StringBuilder sql = new StringBuilder(string.Format(@"/*dialect*/select 
                                   e.F_PYEO_BASE as F_PYEO_HT,
                                   en.F_SRT_PROJECT1 as F_PYEO_project,
	                               t.* into {0}
                            from {1} t left join T_AP_PAYABLE e on t.FPAYABLEBILLNO = e.FBILLNO
                            left join T_AP_PAYABLEENTRY en on t.FENTRYID = en.FENTRYID
                            where 1=1 ", tableName, tempTableName));
            if (!projectId.Equals("0"))
            {
                sql.AppendFormat(" and en.F_SRT_PROJECT1 = '{0}'", projectId);
            }
            if (!ht.Equals("0"))
            {
                sql.AppendFormat(" and e.F_PYEO_BASE = '{0}'", ht);
            }
