...
namespace DFYR.RTJQR.PlauginService.ReverseAmount
{
    ...
    public class ReturnPaymentReverse : AbstractOperationServicePlugIn
    {
        ...
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            foreach (DynamicObject o in e.DataEntitys)
            {
                ...
                string queryPurOrderNoSql = string.Format(@"select e.FPURCHASEORDERNO as FPURORDERNO from T_AP_REFUNDBILL t 
                                                left join T_AP_REFUNDBILLSRCENTRY e on t.FID = e.FID 
                                                where t.FID = '{0}'
                                                group by FPURCHASEORDERNO", id);
                ...
                foreach (DynamicObject purOrderNo in purOrderNoResult)
                {
                    ...
                    string queryAmountSql = string.Format(@"select FORDERENTRYID,sum(e.FREALREFUNDAMOUNTFOR) as amount,e.FPURCHASEORDERNUMBER from T_AP_REFUNDBILL t 
                                                        left join T_AP_REFUNDBILLENTRY e on t.FID = e.FID 
                                                        where e.FPURCHASEORDERNUMBER = '{0}'
                                                        group by e.FPURCHASEORDERNUMBER,FORDERENTRYID", purNo);
                    ...
                    foreach (DynamicObject queryAmount in queryAmountResult)
                    {
                        ...
                        queryAmountData.Add("glfktkje", amount);
